...
const CompleteInformationForm: React.FC<FormProps> = ({
  ...
  const { t } = useFormTranslation('cle.advance.completeInformation');
  const { taskId } = useParams();
  const navigate = useNavigate();

  const { fetchData: fetchRequestData } = useSingleFetch<
    ApiResponse<RequestDetailsDto<CreateApiDataDto>>
  >(`${ApiUrls.cle.advance.completeInformation.get(taskId!, true)}`);
  ...
  const onFinishForm = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        NotificationToast.success(
          <NewCollectionSuccess
            ...
            subtitle={t('title')}
            title={t('requestCreatedSuccessfully', { ns: 'common' })}
          />,
          {
            toastId: 'advance-complete-information-success',
          },
        );
        goBackPage();
      }, 500);
      ...
      NotificationToast.error(getErrorMessage(err), {
        toastId: 'advance-complete-information-error',
      });
    }
  };
