...
import { get as completeInfoResponse } from '../../../../../testUtils/mocks/data/cli/financingRequest/completeInformation';
...
describe('Form CLI FINANCING REQUEST CompleteInformationForm', () => {
  const taskId = 'TSK-123456789';
  const getUrl = `${ApiUrls.cli.financingRequest.completeInformation.get(
    taskId,
    true,
  )}`;
  ...
  it('fetches and loads the data of the request successfully', async () => {
    ...
    expect(screen.getByText(data.request.account.iban)).toBeInTheDocument();
    expect(
