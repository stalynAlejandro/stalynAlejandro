...
import { serializeCliFinancingRequestCreation } from '../../../../helpers/mappers/serializers/cli/financingRequest/serialize';
...
import { summarizeCliFinancingRequestCreation } from '../../../../helpers/mappers/summarizers/summarizeCliFinancingRequestCreation';
...
const CreateForm: React.FC<FormProps> = ({ initialFormData, initialStep }) => {
  ...
  const { fetchData: fetchFinishForm, isLoading: finishFormIsLoading } =
    useSingleFetch<ApiResponse<any>>(
      ApiUrls.cli.financingRequest.create.confirm,
    );
  const { fetchData: fetchOnSaveDraft, isLoading: saveDraftIsLoading } =
    useSingleFetch<ApiResponse<any>>(
      ApiUrls.cli.financingRequest.create.saveDraft,
    );
  ...
  const onSaveDraft = async (userTriggered: boolean = true) => {
    const serializedFormData = await serializeCliFinancingRequestCreation(
      formData,
    );
  ...
  const onFinishForm = async () => {
    ...
    try {
      ...
      NotificationToast.success(
        ...
        />,
        {
          toastId: 'financing-request-created',
        },
      );
      ...
      NotificationToast.error(getErrorMessage(err), {
        toastId: 'financing-request-creation-error',
      });
    }
  };
    ...
    const fetchCollectionData = async () => {
      ...
      try {
        const resp = await fetchServer<ApiResponse<CreateApiDataDto>>(
          ApiUrls.cli.financingRequest.create.get(collectionRef!),
        );
        setIsLoading(false);
    ...
    >
      ...
      <StepComponent
        ...
        onSummarizeFormData={(includedSteps) =>
          summarizeCliFinancingRequestCreation(
