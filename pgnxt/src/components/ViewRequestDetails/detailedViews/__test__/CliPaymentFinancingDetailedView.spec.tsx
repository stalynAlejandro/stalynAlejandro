...
import { get as mockedRequestResponse } from '../../../../testUtils/mocks/data/cli/paymentFinancing/completeInformation';
import * as openNewWindowModule from '../../../../utils/openNewWindow';

describe('Component ViewRequestDetails CliPaymentFinancingDetailedView', () => {
  ...
  const renderWithProps = (props: any = {}) => {
    renderComponentWithToast(
      <CliPaymentFinancingDetailedView {...defaultProps} {...props} />,
    );
  };
  ...
  it('shows the expected detailed information', async () => {
    ...
    expect(await screen.findByText(mockedRequest.code)).toBeInTheDocument();
    expect(
    ...
    expect(
      await screen.findByText(mockedRequest.request.clientAccount.iban),
    ).toBeInTheDocument();
    expect(
      await screen.findByText(mockedRequest.request.commissionAccount.iban),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
